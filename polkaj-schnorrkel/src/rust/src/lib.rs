...
fn sign(message: Vec<u8>, sk: Vec<u8>, pubkey: Vec<u8>) -> Result<Vec<u8>, String> {
    let pubkey = PublicKey::from_bytes(pubkey.as_slice()).map_err(|e| e.to_string())?;

    let signature = SecretKey::from_ed25519_bytes(sk.as_slice())
        .map_err(|e| e.to_string())?
        .sign_simple(SIGNING_CTX, message.as_slice(), &pubkey)
        .to_bytes()
        .to_vec();
    Ok(signature)
}
...
pub extern "system" fn Java_io_emeraldpay_polkaj_schnorrkel_SchnorrkelNative_sign(
    ...
    let output = match sign(message, sk, pubkey) {
        Ok(signature) => env
            .byte_array_from_slice(signature.as_slice())
            .expect("Couldn't create result"),
        Err(msg) => {
            let none = env.new_byte_array(0).expect("Couldn't create empty result");
...
pub extern "system" fn Java_io_emeraldpay_polkaj_schnorrkel_SchnorrkelNative_keypairFromSeed(
    ...
    let seed = env.convert_byte_array(seed).expect("Seed is not provided");
...
pub extern "system" fn Java_io_emeraldpay_polkaj_schnorrkel_SchnorrkelNative_deriveHard(
    ...
    let keypair = env
        .convert_byte_array(keypair)
        .expect("Keypair is not provided");
    let cc = env
        .convert_byte_array(cc)
...
pub extern "system" fn Java_io_emeraldpay_polkaj_schnorrkel_SchnorrkelNative_vrfVerify(
    ...
    let vrf_output_bytes = env
        .convert_byte_array(vrf_output_raw)
        .expect("Vrf output bytes not provided.");

    let vrf_proof_bytes = env
        .convert_byte_array(vrf_proof_raw)
        .expect("Vrf proof bytes not provided.");
...
pub extern "system" fn Java_io_emeraldpay_polkaj_schnorrkel_SchnorrkelNative_vrfSign(
    ...
    let sk_bytes = env
        .convert_byte_array(sk_raw)
        .expect("Secret key bytes not provided.");
...
fn make_bytes<T>(
    ...
    context: &[u8],
    vrf_input: T,
    vrf_pre_output: &VRFPreOut,
) -> Result<[u8; 16], SignatureError>
where
...
pub extern "system" fn Java_io_emeraldpay_polkaj_schnorrkel_SchnorrkelNative_makeBytes(
    ...
    let vrf_output_bytes = env
        .convert_byte_array(vrf_output_bytes)
